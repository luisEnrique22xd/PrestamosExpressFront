...
export default function PrestamosPage() {
  ...
  // Bloquea si tiene crédito activo individual O si tiene deudas pendientes (saldo > 0)
  const tieneBloqueo = clienteEncontrado?.tiene_prestamo_activo || clienteEncontrado?.saldo_actual > 0 || clienteEncontrado?.tiene_moras_activas;
  ...
  const agregarIntegrante = (socio: any) => {
    // RESTRICCIÓN: Si ya tiene deuda (individual o grupal), no puede entrar a un nuevo grupo
    if (socio.tiene_prestamo_activo || socio.saldo_actual > 0) {
      lanzarAlerta('error', `${socio.nombre} tiene deudas activas y no puede ser parte del grupo.`);
      return;
    }
  ...
  const seleccionarGrupoExistente = (g: any) => {
    if (g.tiene_prestamo_activo) {
        lanzarAlerta('error', `El grupo ${g.nombre} ya tiene un crédito activo.`);
        return;
    }
    setFormData({
      ...formData,
  ...
  const buscarCliente = async (id: string) => {
    ...
    try {
      // Usamos el directorio híbrido para obtener datos de deuda en tiempo real
      ...
      if (cliente) {
        ...
        if (cliente.tiene_prestamo_activo || cliente.saldo_actual > 0) {
            lanzarAlerta('error', `Bloqueo: ${cliente.nombre} presenta deudas activas.`);
        }
    ...
    <div className="max-w-4xl mx-auto space-y-8 pb-20 animate-in fade-in duration-500">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          {tipoPrestamo === 'G' ? (
            <div className="col-span-1 md:col-span-2 space-y-6">
                  ...
                  <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-2xl shadow-2xl z-50 border border-slate-100 overflow-hidden">
                    {gruposExistentes.filter(g => g.nombre.toLowerCase().includes(formData.nombre_grupo.toLowerCase())).map(g => (
                      <button key={g.id} type="button" onClick={() => seleccionarGrupoExistente(g)} className="w-full p-4 text-left hover:bg-purple-50 flex items-center justify-between border-b last:border-none">
                        ...
                        {g.tiene_prestamo_activo ? <div className="text-[8px] bg-red-100 text-red-600 px-2 py-1 rounded font-black">DEUDA ACTIVA</div> : <div className="text-[8px] bg-emerald-100 text-emerald-600 px-2 py-1 rounded font-black">DISPONIBLE</div>}
                      </button>
                    ))}
                  </div>
                )}
                  ...
                  <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-2xl shadow-2xl z-50 border border-slate-100 overflow-hidden">
                    {sugerenciasSocios.map(s => (
                      <button key={s.id} type="button" onClick={() => agregarIntegrante(s)} className="w-full p-4 text-left hover:bg-blue-50 border-b flex justify-between items-center">
                        ...
                        {(s.tiene_prestamo_activo || s.saldo_actual > 0) ? <AlertCircle size={14} className="text-red-500" /> : <UserPlus size={14} className="text-[#0047AB]" />}
                      </button>
                    ))}
                  </div>
                )}
              ...
              <div className="flex flex-wrap gap-3">
                {integrantes.map(i => (
                  <div key={i.id} className="flex items-center gap-2 bg-[#0047AB] text-white px-4 py-2 rounded-full text-[10px] font-black uppercase italic">
                    {i.nombre}
            ...
            <div className="space-y-2">
              ...
              {clienteEncontrado && (
                <p className={`text-[10px] font-black uppercase ml-2 italic ${tieneBloqueo ? 'text-red-500' : 'text-emerald-500'}`}>
                    {tieneBloqueo ? '❌ RESTRINGIDO' : '✅ VALIDADO'}: {clienteEncontrado.nombre}
                </p>
              )}
            </div>
          )}
          ...
          <div className="col-span-1 md:col-span-2 p-8 bg-blue-50/30 rounded-[2.5rem] border border-blue-100 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <input type="text" placeholder="Nombre completo" value={formData.nombre_aval} onChange={(e) => setFormData({ ...formData, nombre_aval: e.target.value })} className="p-4 rounded-xl border border-blue-100 outline-none font-bold text-sm" />
              <input type="tel" placeholder="Teléfono" value={formData.telefono_aval} onChange={(e) => setFormData({ ...formData, telefono_aval: e.target.value })} className="p-4 rounded-xl border border-blue-100 outline-none font-bold text-sm" />
          ...
          <div className="space-y-2">
            ...
            <select value={formData.cuotas} onChange={(e) => setFormData({ ...formData, cuotas: e.target.value })} className="w-full p-4 bg-slate-50 rounded-2xl outline-none font-bold text-slate-700">
          ...
          {tieneBloqueo && (
            <div className="col-span-1 md:col-span-2 bg-red-50 border-2 border-red-200 p-6 rounded-[2.5rem] flex items-center gap-5">
              ...
              <div>
                <h4 className="text-red-800 font-black uppercase text-xs italic">Restricción de Crédito Detectada</h4>
                <p className="text-red-500 text-[11px] font-bold">
                  Este cliente/grupo presenta deudas vigentes o moras activas. No se puede autorizar un nuevo folio hasta liquidar.
                </p>
              </div>
            </div>
          )}
      ...
      {alerta && (
        <div className={`fixed top-10 right-10 z-[100] p-6 rounded-[2rem] shadow-2xl flex items-center gap-4 animate-in slide-in-from-right duration-500 border-b-4 ${
          ...
          <div>
            <p className="font-black text-[10px] uppercase tracking-widest text-slate-400 mb-1">Notificación</p>
